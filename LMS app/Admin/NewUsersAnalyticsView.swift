//
//  NewUserAnalyticsView.swift
//  Analytics
//
//  Created by Akash Vishwakarma on 13/06/24.
//
import SwiftUI

struct NewUsersAnalyticsView: View {
    @State private var selectedSegment = 0
    @Environment(\.presentationMode) var presentationMode  // Environment variable to control modal presentation

    private var totalNewUsers: Int {
    ...
    var body: some View {
        NavigationView {
                            ...
                            ForEach(currentStats, id: \.day) { stat in
                                VStack {
                                    Rectangle()
                                        ...
                                        .frame(width: 30, height: self.heightForNewUsers(stat.issues))
                                    Text(stat.shortDay)
                                        .font(.caption)
                                }
                            }
            ...
            .navigationTitle("New Users Analytics")
    ...
    private func heightForNewUsers(_ newUsers: Int) -> CGFloat {
        let maxNewUsers = currentStats.map { $0.issues }.max() ?? 1
        let maxHeight: CGFloat = 200
        return CGFloat(newUsers) / CGFloat(maxNewUsers) * maxHeight
    }
